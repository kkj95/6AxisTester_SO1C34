...
namespace FZ4P
{
    public class AFLinCompCoefAKM7314
    {
        ...
        public int LinCompMain(float[] targPosi, float[] lensPosi, int numData, int pVt, int nVt, int ignInf, int ignMac, ref int[] linCoef, ref float resError)
        {
            ...
            //Data preparation
            ringBufLinComp2 lineData = new ringBufLinComp2();                            // Ring buffer for input data processing
            ...
            printf("\nsarrY13[i], snorY[i], sarrIDEAL[i], sarrIdeal[i]\n");
        ...
        int PreData(ref ringBufLinComp2 lineData, int pVt, int nVt, int ignInf, int ignMac, ref float pPos, ref float nPos)
        {
            //--------------------------------------------------
            //	Check of input data
            //--------------------------------------------------
            ...
            pPos = (-pVt / 512.0f + 0.5f) * 64.0f;
            pPos = (int)pPos / 64.0f;

            nPos = (nVt / 512.0f - 0.5f) * 64.0f;
            nPos = (int)nPos / 64.0f;

            return 0;
        }
        int ConvData(ringBufLinComp2 lineData, int posibit, float pPos, float nPos, float[] arrX13, float[] arrY13, float[] norX, float[] norY, float[] arrIDEAL, float[] arrIdeal, ref float maxX, ref float minX, ref float maxY, ref float minY)
        {
            ...
            for (int i = lineData.startIndex; i < lineData.endIndex + 1; i++)
            {
                dataX[i - lineData.startIndex] = lineData.dataX[i] * AKM_Pow(2.0f, 13 - posibit);
        ...
        int CalLinCompCoef1(ref float[] inputX, ref float[] arrINL0, int[] arrRegCoef1)
        {
            ...
            CheckParam1(ref arrCoef1, ref arrRegCoef1);
        ...
        int CheckParam1(ref float[] arrCoef1, ref int[] arrRegCoef1)
        {
        ...
        int CalINL1(float[] arrX13, float[] arrY13, float[] norX, float[] norY, float[] arrIDEAL, float[] arrIdeal, float[] arrINL1, float[] arrInl1, float[] arrLin1, int[] arrRegCoef1, float pPos, float nPos, float maxX, float minX, float maxY, float minY)
        {
            ...
            if (arrRegCoef1[COEF_X0] < AKM_Pow(2.0f, BIT_COEF10 - 1)) { sarrCoef1[COEF_X0] = arrRegCoef1[COEF_X0] / AKM_Pow(2.0f, BIT_COEF1 - 1); }
            else { sarrCoef1[COEF_X0] = (arrRegCoef1[COEF_X0] - AKM_Pow(2.0f, BIT_COEF10)) / AKM_Pow(2.0f, BIT_COEF1 - 1); }

            for (int i = 1; i < NUM_COEF1 - 1; i++)
            {
                if (arrRegCoef1[i] < AKM_Pow(2.0f, BIT_COEF1 - 1)) { sarrCoef1[i] = arrRegCoef1[i] / AKM_Pow(2.0f, BIT_COEF1 - 1); }
                else { sarrCoef1[i] = (arrRegCoef1[i] - AKM_Pow(2.0f, BIT_COEF1)) / AKM_Pow(2.0f, BIT_COEF1 - 1); }
            }
            ...
            {
                float slopeOut;
                slopeOut = (maxY - minY) / (maxX - minX);
                CalApprox2(sarrX13LNC, arrY13, arrX13, sarrY13LNC, MAX_NUM_DATA, slopeOut);
        ...
        int CalApprox2(float[] inputX, float[] inputY, float[] outputX, float[] outputY, int numData, float slopeOut)
        {
            for (int i = 0; i < numData; i++)
            {
                if (outputX[i] < inputX[0])
                {
                    int k = 0;
                    float offset = inputY[k] - slopeOut * inputX[k];

                    outputY[i] = slopeOut * outputX[i] + offset;
                }
                else
                {
                    for (int k = 0; k < numData - 1; k++)
                    {
                        ...
                        else if (outputX[i] > inputX[numData - 1])
                        {
                            k = numData - 2;
                            float offset = inputY[k] - slopeOut * inputX[k];
        ...
        int CalLinCompCoef2(float[] norX, float[] arrINL1, float[] norY, float[] arrX13, float[] arrY13, float pPos, float nPos, float maxX, float minX, float maxY, float minY, float[] arrIDEAL, float[] arrIdeal, float[] arrLin1, int[] arrRegCoef2, float[] arrINL2, float[] arrLin2, float[] arrInl2)
        {
            ...
            //    int arrRegCoef2_OptSeg[NUM_COEF2_2];
        ...
        int CalINL2(float[] arrX13, float[] arrY13, float[] norX, float[] norY, float[] arrIDEAL, float[] arrIdeal, float[] arrINL2, float[] arrLin2, float[] arrInl2, int[] arrRegCoef2, float pPos, float nPos, float maxX, float minX, float maxY, float minY, float[] arrLin1, int INL)
        {
            ...
            for (int i = 0; i < NUM_COEF2_2 - 1; i++)
            {
                if (arrRegCoef2[NUM_COEF2_1 + i] < AKM_Pow(2.0f, 6)) { sarrNorCoef2[NUM_COEF2_1 + i] = arrRegCoef2[NUM_COEF2_1 + i] / AKM_Pow(2.0f, 10); }
                else { sarrNorCoef2[NUM_COEF2_1 + i] = (arrRegCoef2[NUM_COEF2_1 + i] - AKM_Pow(2.0f, 7)) / AKM_Pow(2.0f, 10); }
            }
